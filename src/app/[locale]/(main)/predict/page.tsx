...
export default function EyeDiagnosisApp() {
  ...
  const handleWebcamCapture = (blob: Blob) => {
    // 1. Chuyển Blob thành File
    const capturedFile = new File([blob], "webcam_capture.jpg", { type: "image/jpeg" });

    // 2. Set vào state chính
    setFile(capturedFile);
    setPreviewUrl(URL.createObjectURL(capturedFile));
    setIsButtonDisabled(false);

    // 3. Tự động chuyển về tab Upload để hiển thị và chẩn đoán
    setActiveTab("upload");
  };
  ...
  return (
    <ProtectedPage>
      <section className="flex flex-col">
        <div className="flex gap-4 mb-4 justify-center">
          <button
            className={`px-4 py-2 rounded-t-md font-semibold ${
              activeTab === "upload" ? "bg-cyan-600 text-white" : "bg-gray-200"
            }`}
          ...
          <button
            className={`px-4 py-2 rounded-t-md font-semibold ${
              activeTab === "webcam" ? "bg-cyan-600 text-white" : "bg-gray-200"
            }`}
                ...
                <div className="bg-gray-50 p-6 rounded-md shadow-inner flex-grow flex flex-col items-center justify-center text-center">
                  {!file && (
                    <p className="text-gray-500 text-lg">
                      Tải lên một bức ảnh để bắt đầu chẩn đoán.
                    </p>
                  )}
        ...
        {activeTab === "webcam" && <FaceTracker onCapture={handleWebcamCapture} />}
